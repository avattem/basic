...
function payRollFieldLengthValidator(element) {
    ...
    if(element.id=="pfAccount"|| element.id =="UANNo"||element.id == "pfno"||element.id=="city"||element.id=="bankAccountNo"||element.id == "employerId"||element.id == "lifeInsureancePolicy"||element.id =="bankAccNo")
    { 
        i=50;
    }
    ...
    if(element.id=="homePhone"||element.id=="mobilePhone"||element.id=="fatherPhone"||element.id == "bankName"||element.id =="workPhone")
    { 
        i=25;
    }
    ...
    if(element.id=="wagecomments"||element.id=="wagecomments1"||element.id=="referencecomments"||element.id=="generalcomments" || element.id=="repaymentComments")
    { 
        i=255;
    }
            
    if(element.id=="passportNo"||element.id=="adharNo")
    { 
        i=20;
    }
   ...
   var temp=0;
    ...
    if(element.id=="PANNumber" ){
        ...
        if(element.value.replace(/^\s+|\s+$/g,"").length>i) {
            ...
            element.value=str.substring(0,i);
            //   x0p( '','Please enter OwnerName!','info');
            x0p( '','Comments length must be less than '+i+' characters','info');
            element.focus();
            return false;
        }
    }
