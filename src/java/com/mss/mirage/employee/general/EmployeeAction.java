...
//            }
            ...
            setCurrentStateHistoryCollection(employeeService.getStateHistoryCollection(getLoginId(), 15));
        ...
        if (httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_USER_ID) != null) {
            ...
            if (AuthorizationManager.getInstance().isAuthorizedUser("DO_UPDATE_STATE_EMP", userRoleId)) {
                ...
                try {
                    ...
                    //  System.out.println("getStateStartDate()"+getStateStartDate());
                    //  Date startDate = getStateStartDate();
                    // System.out.println("resultDate"+result);
                    //  System.out.println("startDate"+startDate);
                    if("Available".equals(getEmpState())){
                     stateCount = DataSourceDataProvider.getInstance().getAvailableState(getId(), getEmpState());
                    ...
                    if (getTempVar() == 2) {
                        setCreatedDate(DateUtility.getInstance().getCurrentMySqlDateTime());
                        if (stateCount > 0) {
                            resultMessage = "<font color=\"red\" size=\"1.5\">Sorry!You cannot add same state consecutively </font>";
                            httpServletRequest.setAttribute(ApplicationConstants.RESULT_MSG, resultMessage);
                        } else {
                            if (result > 0) {
                                resultMessage = "<font color=\"red\" size=\"1.5\">Start Date must be greater than previous status dates</font>";
                                httpServletRequest.setAttribute(ApplicationConstants.RESULT_MSG, resultMessage);
                            } else {
                                ...
                                resultMessage = "<font color=\"green\" size=\"1.5\">Employee State has been successfully updated!</font>";
                                setProjectName("");
                                setSkillSet("");
                                setComments("");
                                if (avaiable > 0) {
                                    resultMessage = "<font color=\"red\" size=\"1.5\">this employee already allocated to project please contact to PMO!</font>";
                                }
                                httpServletRequest.setAttribute(ApplicationConstants.RESULT_MSG, resultMessage);
                            }
                        }
   ...
   /* public String execute()throws FileNotFoundException,ServletException {
    ...
    public String doImageUpdate() throws FileNotFoundException, ServletException {
        ...
        try {
            employeeService = ServiceLocator.getEmployeeService();
            connection = ConnectionProvider.getInstance().getConnection();
            statement = connection.prepareStatement("update tblEmployee set Image=? where LoginId=?");
            statement.setBinaryStream(1, imageFile, (int) imagePath.length());
